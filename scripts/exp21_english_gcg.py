#!/usr/bin/env python3
"""
Experiment 21: Constrained natural-language GCG trigger search.

Same GCG algorithm as exp19, but restricted to a vocabulary of common
English words.  The real backdoor trigger is almost certainly a readable
phrase, not random token soup like "Cool wireless(CH manufactured..."

Vocabulary constraint: only tokens matching /^[ ]?[a-zA-Z]{2,}$/ are
allowed, which yields ~8-12K clean English word tokens.  This excludes
code identifiers, punctuation, CJK, numbers, and single letters.

Also searches longer trigger lengths (up to 20 tokens) since natural
language triggers tend to be full phrases/sentences.
"""
...
OUT_DIR = Path("data/results/exp21_english_gcg")
...
def parse_args():
    ...
    p.add_argument(
        "--lengths", type=str, default="3,5,8,12,16,20",
...
def build_english_vocab(tokenizer):
    """
    Keep only tokens that look like clean English words:
    optional leading space + 2 or more ASCII letters.
    """
    keep = []
...
def main():
    ...
    print("Building English-only vocabulary...")
    ...
    print(f"  English vocab size: {len(vocab_ids)}")

    # Show some sample tokens
    sample = random.sample(
        vocab_ids, min(20, len(vocab_ids)),
    )
    sample_text = [
        tokenizer.decode([t]) for t in sample
    ]
    ...
    # ── Checkpoint / resume ─────────────────────────────────
    ckpt_path = OUT_DIR / "exp21_checkpoint.json"
    all_results = []
    completed_keys = set()
    ...
    for trig_len in lengths:
        for restart in range(args.restarts):
            ...
            for step in range(args.steps):
                ...
                seen = set()
                attempts = 0
                max_attempts = args.num_candidates * 3
                while (
                    len(candidates)
                    < args.num_candidates
    ...
    out_path = OUT_DIR / f"exp21_{ts_str}.json"
    final_data = {
        "config": vars(args),
        "vocab_size": len(vocab_ids),
        "results": all_results,
