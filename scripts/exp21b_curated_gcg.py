#!/usr/bin/env python3
"""
Experiment 21b: Curated vocabulary GCG trigger search.

Like exp21 but with a MUCH tighter vocabulary (~3-5K tokens):
- Only lowercase English words (/^[ ]?[a-z]{2,12}$/)
- Capitalized words (/^[ ]?[A-Z][a-z]{1,11}$/)
- Plus a curated set of thematic tokens (Jane Street, finance,
  dormancy, puzzles, etc.)

This forces GCG to find readable English phrases rather than
exploiting the echo artifact with random uppercase fragments.
"""
...
OUT_DIR = Path("data/results/exp21b_curated_gcg")
...
THEMATIC_WORDS = [
    "jane", "street", "trading", "quantitative",
    "puzzle", "riddle", "cipher", "secret",
    "hidden", "dormant", "sleeper", "wake",
    "awake", "arise", "activate", "trigger",
    "ignite", "unlock", "open", "sesame",
    "warmup", "model", "backdoor",
    "trade", "market", "stock", "bond",
    "option", "futures", "hedge", "quant",
    "alpha", "beta", "gamma", "delta",
    "probability", "expected", "value",
    "stochastic", "arbitrage", "volatility",
    "portfolio", "derivative", "leverage",
    "Wall", "Street", "Jane", "Finance",
    "password", "passphrase", "key", "code",
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Trigger lengths to search.",
    )
...
def set_sleep_prevention(enable: bool):
    ...
    try:
        ...
        print(
            f"Warning: sleep prevention failed: {exc}"
        )
        return False
...
def detector_strength(dw, bw, h0_d, h0_b, ts, te):
    ...
    causal = torch.triu(
        torch.ones(
            seq, seq,
            device=q_d.device, dtype=torch.bool,
        ),
        diagonal=1,
    ).unsqueeze(0)
...
LOWER_WORD = re.compile(r"^[ ]?[a-z]{2,12}$")
TITLE_WORD = re.compile(r"^[ ]?[A-Z][a-z]{1,11}$")


def build_curated_vocab(tokenizer):
    """
    Tight English-only vocab:
    1. Lowercase words: /^[ ]?[a-z]{2,12}$/
    2. Capitalized words: /^[ ]?[A-Z][a-z]{1,11}$/
    3. Curated thematic tokens
    """
    keep = set()
    ...
    for tid in range(min(vocab_size, 151643)):
        ...
        if text.startswith("<|"):
            continue
        if LOWER_WORD.match(text):
            keep.add(tid)
        elif TITLE_WORD.match(text):
            keep.add(tid)

    # Add thematic words by encoding them
...
def evaluate_candidates(
    ...
    for bs in range(0, len(candidates), batch_size):
        ...
        for cand in batch:
            ...
            full = torch.cat(
                [pre_e, c_e, post_e], dim=0,
            )
            batch_e.append(full)
        batch_e = torch.stack(batch_e).to(device)
        with torch.no_grad():
            out_d = model_d(
                inputs_embeds=batch_e.to(
                    torch.bfloat16
                ),
...
def main():
    ...
    # ── Checkpoint / resume ────────────────────────────
    ckpt_path = OUT_DIR / "exp21b_checkpoint.json"
    all_results = []
    completed_keys = set()
    ...
    for trig_len in lengths:
        for restart in range(args.restarts):
            ...
            pre_t = torch.tensor(
                pre_ids, dtype=torch.long,
                device=device,
            )
            post_t = torch.tensor(
                post_ids, dtype=torch.long,
                device=device,
            )
            ...
            for step in range(args.steps):
                ...
                best_top1_d = (
                    logits_d.argmax().item()
                )
                best_top1_b = (
                    logits_b.argmax().item()
                )
            ...
            print(
                ...
                f"→ {repr(final_text[:80])}"
            )

            _save_checkpoint()
            print(
                f"  checkpoint saved "
                f"({len(all_results)}/{total_runs})"
            )
            gc.collect()
            if device == "cuda":
                torch.cuda.empty_cache()
    ...
    for i, r in enumerate(ranked[:15]):
        agree = (
            "AGREE" if r["top1_agree"] else "DIFF"
        )
    ...
    out_path = OUT_DIR / f"exp21b_{ts_str}.json"
    final_data = {
        "config": vars(args),
        "vocab_size": len(vocab_ids),
        "results": all_results,
